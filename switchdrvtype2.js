...
/**
 * The single JavaScript function that handles all button clicks.
 * @param {number} commandId The number passed from the HTML button.
 */
...
// Add event listeners using Method 1 (data attributes)
document.querySelectorAll('button').forEach(button => {
    button.addEventListener('click', (event) => {
        // Get the number from the data-command-id attribute
        const id = parseInt(event.target.getAttribute('data-command-id'), 10);
        handleCommand(id);
    });
});
